...
_Incorrect: Segregating those compute nodes to their own OpenStack clusters complicates communication between different kinds of server instances of the applications._

* [ ] Compute nodes with the accelerator card belong to an AZ. +
_Incorrect: hardware capabilities of compute nodes to not configure a failure domain._
...
_Incorrect: hardware capabilities of compute nodes to not configure a compute cell._
...
_Incorrect: There is no need to create a host aggregate for the intersection of different hardware capabilities because a compute node can belong to multiple host aggregates._
