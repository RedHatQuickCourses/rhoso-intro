...
Thanks to Placement, Nova can run each server instance in a compute node with sufficient available compute reources for that instance.
...
Defining classse of compute resources is usually an OpenStack Admninistrator task, while configuring workloads to consume compute resources from those classes is an OpenStack Operator task.
