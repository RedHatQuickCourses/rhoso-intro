...
1. Which of the following components is either included or required by Red Hat OpenStack Services on OpenShift?
...
_Incorrect: Red Hat Open Services on OpenShift do not include and do not support running a control plane on RHV. It also does not support running application workloads on VMs managed by RHV._

* [*] *Red Hat Ceph Storage* +
_Correct: Ceph is NOT required but it is optionally bundled and supported for customers that need a software-defined storage solution._
...
* [ ] Open vSwitch +
_Incorrect: vSwitch was used directly in older releases of the Red Hat OpenStack platform but is only used internally by OVN in Red Hat OpenStack Services on OpenShift._
...
3. Where does an OpenStack on OpenShift cluster run application workloads?
...
4. How does Red Hat OpenStack Services on OpenShift ensure that OpenStack control planes are reliable and scalable?

* [ ] By running control plane servers as VMs using a standalone hypervisor such as RHV. +
_Incorrect: This was an option with older releases of the Red Hat OpenStack platform, but not anymore with Red Hat OpenStack Services on OpenShift._
